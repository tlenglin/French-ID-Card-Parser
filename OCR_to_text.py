...
def ocr():
    ...
    print lines
    if len(lines) > 0:
        cleaned_MRZ[0] = line_fixer1(lines["1"])
    if len(lines) > 1:
        cleaned_MRZ[1] = line_fixer2(lines["2"])
    return cleaned_MRZ
...
if s0 < 88:
    ...
    if s1 < 88:
        ...
        if s2 < 88:
            ...
            new_MRZ = ocr()
            aligned_MRZ = align_lines(aligned_MRZ, new_MRZ[0], new_MRZ[1])
            s3 = score(aligned_MRZ[0], aligned_MRZ[1])
            if s3 < 88:
                MRZ_crop(190)
                new_MRZ = ocr()
                aligned_MRZ = align_lines(aligned_MRZ, new_MRZ[0], new_MRZ[1])
                s4 = score(aligned_MRZ[0], aligned_MRZ[1])
                print "s4 = ", s4
                if s4 < 88:
                    if s4 > 50:
                        print "uncertain result, 50 < s4 < 80"
                    else:
                        print "probably no result s4 < 50"
